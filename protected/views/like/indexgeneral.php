...
<?php $this->setPageTitle("ESTADÍSTICAS GUSTADOS"); ?>
...
<div class="row">
    ...
    <div class="panel-heading">
        <i class="fa fa-fw fa-thumbs-o-up"></i> Recursos Gustados Total  
...
<div class="col-md-10 alpha omega ">	
    ...
    <div class="panel-heading">
        <i class="fa fa-bar-chart-o fa-fw"></i> Recursos Gustados Total por Meses  
        <div style="float: right;"> 
...
<div class="col-md-10 alpha omega ">	
    ...
    <div class="panel-heading">
        <i class="fa fa-bar-chart-o fa-fw"></i> Recursos Gustados Total por Días
         <div style="float: right;"> 
	        <label>Tipo de Gráfica: </label>
...
<script type="text/javascript">
		...
		jQuery(document).ready(function($){
			...
			$("#btnBuscar").on("click",function () {
				...
				$.ajax({
        		type: "post",
        		url: "/gamma/like/JSONtodosgustadospormesrango/",	        		
		        dataType: "json",
		        data: {'fecha1':$("#fecha1").val(), 'fecha2':$("#fecha2").val()},
		        success: function(response, status) {
		        	var html=null; 
		        	html="<table id='tblMesesGustados' border='1' class='table table-bordered table-striped results'><thead><th>Mes</th><th>Total Visitas</th></thead><tbody>";       	
		        	var b = new Array();
                               var fila=0;
			                     for (var i = 0; i < response.length; i++) {
			                         html=html+"<tr><td><div class='mes' style='color:blue;'>"+response[i]['mes']+"</div></td><td>"+response[i]['gustado']+"</td></tr>";
			                         b[fila] = new Array();
			                         b[fila]["y"]=response[i]['mes'];
			                         b[fila]["a"] = response[i]['gustado'];            			                         		                         
					...
					encabezado= encabezado+"<?php echo "Total Gustados por Meses                  "; ?>";
					encabezado= encabezado+"<?php echo "									      "; ?>";
					encabezado= encabezado+"<?php echo "						        		  "; ?>";		
					            ...
					            "aButtons": [
					                ...
					                },
					                {
					                    ...
					                    "aButtons":    [ "csv","xls", { "sExtends": "pdf",	                    
			                    										 ...
			                    										 "sFileName":"totalgustadospormeses.pdf"
							                    						}]
					                }
					            ]
					        }
			...
			$(document).on('click','.mes', function () {	
				...
				$.ajax({
        		type: "post",
        		url: "/gamma/like/JSONtodosgustadospordias/",	        		
		        dataType: "json",
		        data: {'mes':$(this).text()},
		        success: function(response, status) {
		        	var html=null; 
		        	html="<table id='tblDiasGustados' border='1' class='table table-bordered table-striped results'><thead><th>Día</th><th>Total Visitas</th></thead><tbody>";       	
		        	var b = new Array();
                               var fila=0;
			                     for (var i = 0; i < response.length; i++) {
			                         html=html+"<tr><td><div class='dia' style='color:blue;'>"+response[i]['dia']+"</div></td><td>"+response[i]['gustado']+"</td></tr>";
			                         b[fila] = new Array();
					...
					encabezado= encabezado+"<?php echo "SOCIAL MEDIA ACADEMIC NETWORK"; ?>";
					encabezado= encabezado+"<?php echo "						        					        "; ?>";
					encabezado= encabezado+"<?php echo "Total Gustados por Días"; ?>                      ";
					encabezado= encabezado+"<?php echo "									      "; ?>";
					encabezado= encabezado+"<?php echo "						        		 "; ?>";		
					encabezado= encabezado+"Fecha: <?php print(date('m/d/Y h:i:s a', time())); ?>";
					            ...
					            "aButtons": [
					                ...
					                },
					                {
					                    ...
					                    "aButtons":    [ "csv","xls", {"sExtends": "pdf",	                    
			                    										 "sPdfMessage": encabezado, 
			                    										 "sTitle": "                                  UNIVERSIDAD TÉCNICA DE AMBATO",
			                    										 "sFileName":"totalgustadospordias.pdf"
							                    						}]
					                }
					            ]
					        }
			...
			$.ajax({
        		type: "post",
        		url: "/gamma/like/JSONgustadostodos",	        		
		        dataType: "json",
		        success: function(response, status) {
		        	var html2='<div class="chart" id="compartidosporanio" style="width:100%; height: 300px;"></div><div class="chart" id="tblTotal" ></div>';
					$("#panel1").html('');		
					$("#panel1").html(html2);   
		        	var html="<table id='tblTotalGustados' border='1' class='table table-bordered table-striped results'><thead><th>Año</th><th>Total Gustados</th></thead><tbody>";      	
		        	var b = new Array();
                               var fila=0;
			                     for (var i = 0; i < response.length; i++) {
					...
					encabezado= encabezado+"<?php echo "SOCIAL MEDIA ACADEMIC NETWORK"; ?>";
					encabezado= encabezado+"<?php echo "						        					        "; ?>";
					encabezado= encabezado+"<?php echo "Total Gustados                              "; ?>";
					encabezado= encabezado+"<?php echo "									           "; ?>";
					encabezado= encabezado+"<?php echo "						        			   "; ?>";		
					encabezado= encabezado+"Fecha: <?php print(date('m/d/Y h:i:s a', time())); ?>";	
	                
	                var table = $('#tblTotalGustados').DataTable( {
							"paging":   true,
					        "ordering": true,
					        "info":     true,
					        'searching':true,
			...
			$("#ddlLimit").on("change",function () {
				...
				{
					url1="/gamma/like/JSONgustadostodos";
				}
				else
				{
					url1="/gamma/like/JSONgustadosporanios";
				}
