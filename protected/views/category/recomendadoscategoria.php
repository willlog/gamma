...
<?php $this->setPageTitle("RECOMENDADOS POR CATEGORÍA"); ?>
...
<div class="col-md-12 alpha omega ">	
    ...
    <div class="panel-heading">
        <i class="fa fa-bar-chart-o fa-fw"></i> Recursos Educativos Recomendados de Todos los Tiempos
...
<div class="col-md-12 alpha omega ">	
    ...
    <div class="panel-heading">
        <i class="fa fa-bar-chart-o fa-fw"></i> Recursos Educativos Recomendados por Fechas
        <div style="float: right;"> 
	        <label>Top: </label>
...
<script type="text/javascript">
		...
		jQuery(document).ready(function($){
			...
			$.ajax({
	        		type: "post",
	        		url: "/gamma/category/JSONrecomendadoscategoria/"+$("#catid").val(),	        		
			        dataType: "json",
			        success: function(response, status) {
			        	var html2='<div class="chart" id="recomendados" style="width:100%; height: 300px;"></div><div class="chart" id="tblRecomendados" ></div>';
						$("#panel1").html('');		
						$("#panel1").html(html2);   
			        	var html="<table id='tblTotalRecomendados' border='1' class='table table-bordered table-striped results'><thead><th>Recurso Educativo</th><th>Autor</th><th>Compartidos</th><th>Visitados</th><th>Gustados</th><th>Relevancia</th></thead><tbody>";     	
			        	var b = new Array();
	                               var fila=0;
				                     for (var i = 0; i < response.length; i++) {
				                         html=html+"<tr><td><a href='"+response[i]['url']+"' target='_blank'>"+response[i]['title']+"</a></td><td>"+response[i]['autor']+"</td><td>"+response[i]['share']+"</td><td>"+response[i]['visit']+"</td><td>"+response[i]['likes']+"</td><td>"+response[i]['recomendado']+"</td></tr>";
				                         b[fila] = new Array();
					            ...
					            "aButtons": [
					                ...
					                },
					                {
					                    ...
					                    "aButtons":    [ "csv","xls", {
													                    ...
													                    "sFileName":"recomendadoscategoria.pdf"
													                }]
					                }
					            ]
					        }
				...
				$("#btnBuscar").on("click",function () {
					var html2='<div class="chart" id="recomendadosmeses" style="width:100%; height: 300px;"></div><div class="chart" id="tblRecomendadosmeses" ></div>';
					$("#panel2").html('');		
					$("#panel2").html(html2);
					$.ajax({
	        		type: "post",
	        		url: "/gamma/category/JSONrecomendadoscategoriarango/"+$("#catid").val(),	        		
			        dataType: "json",
			        data: {'fecha1':$("#fecha1").val(), 'fecha2':$("#fecha2").val()},
			        success: function(response, status) {			        	 
			        	var html="<table id='tblMesesRecomendados' border='1' class='table table-bordered table-striped results'><thead><th>Recurso Educativo</th><th>Autor</th><th>Compartidos</th><th>Visitados</th><th>Gustados</th><th>Relevancia</th></thead><tbody>";     	
			        	var b = new Array();
	                               var fila=0;
				                     for (var i = 0; i < response.length; i++) {
				                         html=html+"<tr><td><a href='"+response[i]['url']+"' target='_blank'>"+response[i]['title']+"</a></td><td>"+response[i]['autor']+"</td><td>"+response[i]['share']+"</td><td>"+response[i]['visit']+"</td><td>"+response[i]['likes']+"</td><td>"+response[i]['recomendado']+"</td></tr>";
				    ...
				    html=html+"</tbody> </table>";	
			        $("#tblRecomendadosmeses").html(html);	        
			        	
			        	//BAR CHART
	                var bar = new Morris.Bar({
	                    element: 'recomendadosmeses',
	                    resize: true,
	                    data: b,
	                    //barColors: 'auto',
	                    xkey: 'y',
					...
					encabezado= encabezado+"<?php echo "							        					        "; ?>";
					encabezado= encabezado+"<?php echo "							        			        "; ?>";
					encabezado= encabezado+"<?php echo "Recursos Recomendados por Categoria                      "; ?>";
					encabezado= encabezado+"<?php echo "		  	                                                                    		        "; ?>";
					encabezado= encabezado+"<?php echo "					        			 "; ?>";		
					encabezado= encabezado+"Fecha: <?php print(date('m/d/Y h:i:s a', time())); ?> ";
					encabezado= encabezado+"Categoría: <?php echo $model->name; ?>                                             ";
	                ...
	                var table = $('#tblMesesRecomendados').DataTable( {
							"paging":   true,
					        "ordering": true,
					        "info":     true,
					        'searching':true,
					            ...
					            "aButtons": [
					                ...
					                },
					                {
					                    ...
					                    "aButtons":    [ "csv","xls", {
													                    ...
													                    "sPdfOrientation": "landscape",            
													                    "sTitle": "                               											                             	   UNIVERSIDAD TÉCNICA DE AMBATO",
													                    "sFileName":"recomendadoscategoriafechas.pdf"
													                	}]
					                }
					            ]
					        }
