...
<div class="row">
    ...
    <div class="panel-heading">
        <i class="fa fa-envelope-o"></i> MENSAJE ENVIADO    
    ...
    <table  width="100%">
		...
		<?php foreach ($dataProvider as $key => $value) { ?>
			<tr>		
				<td width="50px"><label>Para: </label></td>		
				<td><input type="email" class="form-control" name="emailto" value="<?php echo $value['name']; ?>" disabled>
					<input id ="userid" value="<?php echo $value['idreceive']; ?>" hidden="hidden" />
				</td>
			</tr>
			<tr>		
			...
			<tr>		
				...
				<td id="estid"><input  type="email" class="form-control" name="emailto" value="<?php echo $value['datesend']; ?>" disabled></td>
			</tr>
			<tr>		
	...
	<div class="box-footer clearfix">
        <button class="pull-right btn btn-default" id="btnResponder"><i class="fa fa-mail-reply"></i> Reenviar </button>
    </div>
    <!-- /.panel-body -->    
    <!-- /.panel-footer -->
    <br>
...
jQuery(document).ready(function($){		
		...
		$("#btnResponder").on("click",function () {
			...
			$("#btnEnviar").on("click",function () {												
				...
				data: {'idsend':$("#myid").val(), 'idreceive':$("#userid").val(), 'subject':$("#subject").val(), "text":$("#panel1").text()},				
	        		success: function(response, status) {	        	
			        	$("#user").val("");	
			        	 $("#lblUser").text("");			        	     	
			        },
