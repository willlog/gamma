...
<?php $this->setPageTitle("RECURSOS RECOMENDADOS"); ?>
...
<script type="text/javascript">
			...
			doc.save("recomendadosautor.pdf");
			...
			"use strict";				
				$.ajax({					
	        		type: "post",
	        		url: "/gamma/user/JSONrecomendadosautor/"+$("#artid").val(),	        		
			        dataType: "json",
			        success: function(response, status) {   
			        	var html2='<div class="chart" id="estadistica" style="width:100%; height: 300px;"></div><div class="chart" id="tblEstadistica" ></div>';
						$("#panel1").html('');		
						$("#panel1").html(html2);
			        	var html="<table id='tblTotalRecomendados' border='1' class='table table-bordered table-striped results'><thead><th>Recurso Educativo</th><th>Compartidos</th><th>Visitas</th><th>Gustados</th><th>Relevancia</th></thead><tbody>";     	
			        	var b = new Array();
	                               var fila=0;
				                     for (var i = 0; i < response.length; i++) {
				                         html=html+"<tr><td><a href='"+response[i]['url']+"' target='_blank'>"+response[i]['title']+"</a></td><td>"+response[i]['share']+"</td><td>"+response[i]['visit']+"</td><td>"+response[i]['likes']+"</td><td>"+response[i]['recomendado']+"</td></tr>";
				                         b[fila] = new Array();
					...
					encabezado= encabezado+"<?php echo "							        					        "; ?>";
					encabezado= encabezado+"<?php echo "Recursos Recomendados por Usuario"; ?>";
					encabezado= encabezado+"<?php echo "							        	                                                                    		        "; ?>";
					encabezado= encabezado+"<?php echo "							        			 "; ?>";		
					            ...
					            "aButtons": [
					                ...
					                },
					                {
					                    ...
					                    "aButtons":    [ "csv","xls", {
													                    ...
													                    "sFileName":"recomendadosautor.pdf"
													                }]
					                }
					            ]
					        }
				...
				$("#ddlLimit").on("change",function () {					
					var html2='<div class="chart" id="estadistica" style="width:100%; height: 350px;"></div><div class="chart" id="tblEstadistica" ></div>';
					$("#panel1").html('');		
					$("#panel1").html(html2);
					$.ajax({
		        		type: "post",
				...
				$("#btnBuscar").on("click",function () {
					var html2='<div class="chart" id="estadisticames" style="width:100%; height: 350px;"></div><div class="chart" id="tblRango" ></div>';
					$("#panel2").html('');		
					$("#panel2").html(html2);
					$.ajax({
		        		type: "post",
		        		url: "/gamma/user/JSONrecomendadosautorrango/"+$("#artid").val(),	        		
				        dataType: "json",
				        data: {'fecha1':$("#fecha1").val(), 'fecha2':$("#fecha2").val()},
				        success: function(response, status) {   
				        	var html="<table id='tblMesesRecomendados' border='1' class='table table-bordered table-striped results'><thead><th>Recurso Educativo</th><th>Compartidos</th><th>Visitas</th><th>Gustados</th><th>Relevancia</th></thead><tbody>";     	
				        	var b = new Array();
		                               var fila=0;
					                     for (var i = 0; i < response.length; i++) {
					                         html=html+"<tr><td><a href='"+response[i]['url']+"' target='_blank'>"+response[i]['title']+"</a></td><td>"+response[i]['share']+"</td><td>"+response[i]['visit']+"</td><td>"+response[i]['likes']+"</td><td>"+response[i]['recomendado']+"</td></tr>";
					            ...
					            "aButtons": [
					                ...
					                },
					                {
					                    ...
					                    "aButtons":    [ "csv","xls", {
													                    ...
													                    "sPdfOrientation": "landscape",            
													                    "sTitle": "                               											                             	   UNIVERSIDAD TÉCNICA DE AMBATO",
													                    "sFileName":"recomendadosautorfechas.pdf"
													                	}]
					                }
					            ]
					        }
