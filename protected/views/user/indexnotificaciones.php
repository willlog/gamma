...
<?php $this->setPageTitle("NOTIFICACIONES"); ?>
<section class="content">   
<div class="col-sm-12">
	<?php echo CHtml::link('<i class="fa fa-warning"> Personalizar mis Notificaciones</i>', array('/user/confignotificacionesusuario'), array('class'=>'btn btnVistaArticle'));  ?>
	<br/>
	<br/>
	<small class="badge bg-blue"><h4><code id="namepertenese">NOTIFICACIONES</code></h4></small>
	<br/>
	<br/> 
	...
	<div class="table-responsive" id="tableArticles">
	            <table id="tblArticle" class="table table-bordered table-striped results">
					...
					<tr>
	                    ...
	                    <td>
	                    	<div class="btn-group">       
	                            <?php echo CHtml::link('<i class="fa fa-eye"></i>', array('/user/vernotificacion/'.$value['idNotificacion']), array('target'=>'_blank', 'class'=>'btn btn-info btnVistaArticle'));  ?>                          
								<button type="button" class="btn btn-danger btnEliminarArticle" value="<?php echo $value['idNotificacion']; ?>"  data-toggle="modal" data-target="#modalDeleteArticles"><i class="fa fa-trash-o"></i></button>
	                        </div>
	                    </td>
	                </tr>
...
<script type="text/javascript">
		...
		$(".btnEliminarArticle").on("click",function () {
			...
			{			
				$.ajax({				
		        		type: "POST",        		
		        		url: "/gamma/user/JSONeliminarnotificacion/"+$(this).attr("value"),        		
						//data: {'idNotificacion':$(this).attr("value")},				
			        		success: function(response, status) {								        	
