    ...
    <table width="100%">
		...
		<?php foreach ($dataProvider as $key => $value) { ?>
			...
			<tr>		
				...
				<td><input type="text" id="subject" value="<?php echo $value['valor']; ?>" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;"></td>
			</tr>		
			<tr>		
				<td width="130px"></td>		
				<td></td>
...
jQuery(document).ready(function($){		
		...
		$("#btnGuardar").on("click",function () {			
			$.ajax({				
                    ...
                    url: "/gamma/user/JSONUpdatevalornotificacion/"+$("#artid").val(),
                    success: function(response, status) {
                    	alert('Hola');                      
                    },
                    error: function (response, status) {
