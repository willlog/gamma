...
<div class="col-md-11 alpha omega ">	
    ...
    <div class="panel-heading">    	
        <i class="fa fa-bar-chart-o fa-fw"></i> Mas Visitados de Todos los Tiempos 
...
<div class="col-md-11 alpha omega ">	
    ...
    <div class="panel-heading">    	
        <i class="fa fa-bar-chart-o fa-fw"></i> Mas Visitados por Fechas 
...
<script type="text/javascript">
		...
		jQuery(document).ready(function($){
			"use strict";				
				$.ajax({					
	        		type: "post",
	        		url: "/gamma/article/JSONMasvisitadostotal/",	        		
			        dataType: "json",
			        success: function(response, status) {   
			        	var html2='<div class="chart" id="estadistica" style="width:100%; height: 300px;"></div><div class="chart" id="tblEstadistica" ></div>';
						$("#panel1").html('');		
						$("#panel1").html(html2);
			        	var html="<table id='tblTotalVisitas' border='1' class='table table-bordered table-striped results'><thead><th>Título</th><th>Creador</th><th>Visitas</th></thead><tbody>";     	
			        	var b = new Array();
	                               var fila=0;
				                     for (var i = 0; i < response.length; i++) {
				                         html=html+"<tr><td><a href='"+response[i]['url']+"' target='_blank'>"+response[i]['title']+"</a></td><td>"+response[i]['name']+"</a></td><td>"+response[i]['visit']+"</td></tr>";
				                         b[fila] = new Array();
					...
					encabezado= encabezado+"<?php echo "							        					        "; ?>";
					encabezado= encabezado+"<?php echo "Recursos Más Visitados de Todos los Tiempos"; ?>";
					encabezado= encabezado+"<?php echo "					                                                          		        "; ?>";
					encabezado= encabezado+"<?php echo "							        			 "; ?>";		
					            ...
					            "aButtons": [
					                ...
					                },
					                {
					                    ...
					                    "aButtons":    [ "csv","xls", {
													                    ...
													                    "sFileName":"masvisitadostodoslostiempos.pdf"
														                }]
					                }
					            ]
					        }
				...
				$("#btnBuscar").on("click",function () 
				{
					$.ajax({
		        		type: "post",
		        		url: "/gamma/article/JSONMasvisitadosrango/",	        		
				        dataType: "json",
						...
						$("#panel2").html(html2);  
				        var html=null; 
			        	html="<table id='tblMesesVisitas' border='1' class='table table-bordered table-striped results'><thead><th>Título</th><th>Creador</th><th>Visitas</th></thead><tbody>";      	
				        	var b = new Array();
		                               var fila=0;
					                     for (var i = 0; i < response.length; i++) {
					                         html=html+"<tr><td><a href='"+response[i]['url']+"' target='_blank'>"+response[i]['title']+"</a></td><td>"+response[i]['name']+"</a></td><td>"+response[i]['visit']+"</td></tr>";
							...
							encabezado= encabezado+"<?php echo "							        					        "; ?>";
							encabezado= encabezado+"<?php echo "							        			        "; ?>";
							encabezado= encabezado+"<?php echo "Recursos Más Visitados por Fechas               "; ?>";
							encabezado= encabezado+"<?php echo "		  	                                                                    		        "; ?>";
							encabezado= encabezado+"<?php echo "					        			 "; ?>";		
							encabezado= encabezado+"Fecha: <?php print(date('m/d/Y h:i:s a', time())); ?>         ";												        			                         "; ?>";
						            ...
						            "aButtons": [
						                ...
						                },
						                {
						                    ...
						                    "aButtons":    [ "csv","xls", {
														                    ...
														                    "sPdfOrientation": "landscape",            
														                    "sTitle": "                               											                             	   UNIVERSIDAD TÉCNICA DE AMBATO",
														                    "sFileName":"masvisitadosporfechas.pdf"
															                }]
						                }
						            ]
						        }
